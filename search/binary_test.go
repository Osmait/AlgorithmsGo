...
func TestBinary(t *testing.T) {
	for _, test := range searchTests {
		...
		if actualValue != test.expected {
			t.Errorf("test '%sfailed:input array '%v' with key '%d',exprected '%d',get,'%d'", test.name, test.data, test.key, test.expected, actualValue)
		}
...
func TestBinaryIterative(t *testing.T) {
	for _, test := range searchTests {
		actualValue, actualError := BinaryIterrative(test.data, test.key)
		if actualValue != test.expected {
